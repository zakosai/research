...
class Dataset(object):
    ...
    @classmethod
    def from_path2(cls, path, file):
        data = list(open(os.path.join(path, file)))
        data = [t.strip().split() for t in data]

        num_users = len(data)
        num_items = len(list(open(os.path.join(path, "item_id.txt"))))
        sequence = dict()

        for user in range(1, num_users+1):
            sequence[user] = np.array([int(i)+1 for i in data[user-1][1:-1]])
        return cls(num_users, num_items, sequence)