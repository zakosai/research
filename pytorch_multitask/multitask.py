...
def main():
    ...
    x_train, y_train, x_test, y_test = map(torch.tensor, (user_item.astype(np.double), dataset['user_onehot'].astype(
        np.double), x_test.astype(np.double),y_test.astype(np.double)))
