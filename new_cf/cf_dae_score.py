...
class MLP(nn.Module):
    ...
    def forward(self, z_user, z_item, user_ids=None, item_ids=None):
        ...
        _x = torch.cat((z_user, z_item), axis=-1)
        return self.net(_x)
...
def train(data, model, op, loss, device):
    ...
    # # NeuCF
    pred = model['neuCF'](z_user, z_item)
...
def test(data, model, device):
    with torch.no_grad():
        ...
        user_recon, z_user = model['user'](user_info)
        item_recon, z_item = model['item'](item_info)
        rating = torch.Tensor(range(1,6)).T.to(device)
...
def main(args):
    ...
    model['neuCF'] = MLP([100, 20, 5], 50, 50, dataset.no_user, dataset.no_item)
