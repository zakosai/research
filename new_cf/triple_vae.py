...
class Translation:
    ...
    def build_model(self):
        ...
        self.loss_user = tf.reduce_mean(tf.reduce_sum(binary_crossentropy(self.user_info, user_recon), axis=1)) +\
                      loss_kl_user + tf.losses.get_regularization_loss(scope="user")
        ...
        self.loss_item = tf.reduce_mean(tf.reduce_sum(binary_crossentropy(self.item_info, item_recon), axis=1)) +\
                        0.1 * loss_kl_item + 0.1 * tf.losses.get_regularization_loss(scope="item")
        ...
        self.loss = self.loss_reconstruct(self.x, self.x_recon) + 2 * tf.losses.get_regularization_loss(scope='CF')

        self.train_op = tf.train.AdamOptimizer(self.learning_rate).minimize(self.loss)
        self.train_op_user = tf.train.AdamOptimizer(1e-4).minimize(self.loss_user)
        self.train_op_item = tf.train.AdamOptimizer(1e-4).minimize(self.loss_item)
