...
recalls_1, mapks_1 = model.predict_val(pred, data['train_users'], data['test_users'], 100)

model.load_model(os.path.join(ckpt, "vae_user.mat"))
pred = model.predict_all()
recalls_2, mapks_2 = model.predict_val(pred, data['train_users'], data['test_users'], 100)

model.load_model(os.path.join(ckpt, "dae.mat"))
# model.load_model("cf_vae.mat")
pred = model.predict_all()
recalls, mapks= model.predict_val(pred, data['train_users'], data['test_users'], 100)
#
# plt.figure()
# plt.ylabel("Recall@M")
# plt.xlabel("M")
