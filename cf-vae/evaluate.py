...
recalls, precision, mapk = model.predict(pred, data['train_users'], data['test_users'], 40)

model.load_model("pre_model/zdim2/cf_vae_1.mat")
pred = model.predict_all()
recalls_1, precision_1, mapk_1 = model.predict(pred, data['train_users'], data['test_users'], 40)
...
plt.savefig("result/recall_zdim2.png")
plt.close()

plt.figure()
plt.ylabel("Precision@M")
plt.xlabel("M")
plt.plot(np.arange(5, 40, 5),precision, '-b', label="cvae")
plt.plot(np.arange(5, 40, 5), precision_1, '-r', label="img-extend")
# plt.plot(np.arange(5, 40, 5), recalls_2, '-g', label="zdim=500")

plt.legend(loc='upper left')
plt.savefig("result/precision_zdim2.png")
plt.close()

plt.figure()
plt.ylabel("MAP@M")
plt.xlabel("M")
plt.plot(np.arange(5, 40, 5),mapk, '-b', label="cvae")
plt.plot(np.arange(5, 40, 5), mapk_1, '-r', label="img-extend")
# plt.plot(np.arange(5, 40, 5), recalls_2, '-g', label="zdim=500")

plt.legend(loc='upper left')
plt.savefig("result/mapk_zdim2.png")
plt.close()