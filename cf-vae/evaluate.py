...
recalls, precision, mapk = model.predict(pred, data['train_users'], data['test_users'], 10)

model.load_model("pre_model/zdim2/cf_vae_1.mat")
pred = model.predict_all()
recalls_1, precision_1, mapk_1 = model.predict(pred, data['train_users'], data['test_users'], 10)
...
plt.savefig("result/recall_zdim2_v2.png")
plt.close()
...
plt.legend(loc='upper left')
plt.savefig("result/precision_zdim2_v2.png")
plt.close()

plt.figure()
plt.ylabel("MAP@M")
plt.xlabel("M")
plt.plot(np.arange(1, 10, 1),mapk, '-b', label="cvae")
plt.plot(np.arange(1, 10, 1), mapk_1, '-r', label="img-extend")
# plt.plot(np.arange(5, 40, 5), recalls_2, '-g', label="zdim=500")

plt.legend(loc='upper left')
plt.savefig("result/mapk_zdim2_v2.png")
plt.close()