...
def main(args):
    ...
    print("A: Train: %f, Test: %f" %(((user_A_train * item_A_popular).mean(axis=1)/user_A_train.sum(axis=1)).mean(),
                                     ((user_A_test * item_A_popular).mean(axis=1)/user_A_test.sum(axis=1)).mean()))
    print("B: Train: %f, Test: %f" % (((user_B_train * item_B_popular).mean(axis=1)/user_B_train.sum(axis=1)).mean(),
                                      ((user_B_train * item_B_popular).mean(axis=1)/user_B_train.sum(axis=1)).mean()))
