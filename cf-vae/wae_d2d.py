...
class Translation:
    ...
    def build_model(self):
        ...
        # VAE for domain A
        z_A = self.encode(x_A, "A", self.encode_dim_A, False, False)
        y_AA = self.decode(z_A, "A", self.decode_dim_A, False, False)

        # VAE for domain B
        z_B = self.encode(x_B, "B", self.encode_dim_B, False, True, True)
        ...
        # Loss VAE
        loss_VAE_A = self.loss_reconstruct(x_A, y_AA)
        loss_VAE_B = self.loss_reconstruct(x_B, y_BB)
        ...
        self.loss_gen = self.loss_val_a + self.loss_val_b + 0.1 * tf.losses.get_regularization_loss() +\
...
def main(args):
    ...
    for i in range(1, iter):
        ...
        if i%10 == 0:
            model.train = False
            print("Loss last batch: loss gen %f, loss dis %f, loss vae %f" % (
            loss_gen, loss_dis, loss_vae))
