...
class cf_vae_extend:
    def __init__(self, num_users, num_items, num_factors, params, input_dim, encoding_dims, z_dim, decoding_dims, encoding_dims_str,
                 decoding_dims_str, loss_type="cross_entropy", useTranse = False, eps = 1e-10, model=0, ckpt_folder="pre3/dae", initial=None, user_dim=3883):
    ...
    def e_step(self, x_data, im_data, str_data, u_data):
        ...
        with tf.variable_scope("user"):
            ...
            for i in range(depth_inf):
                ...
                # print("enc_layer0/weights:0".graph)
            h_u_encode = x_u
            z_u = dense(h_u_encode, self.z_dim, scope="mu_layer", activation=tf.nn.sigmoid)
        ...
        if self.initial:
            ...
            ckpt_file = os.path.join(self.ckpt, "dae_user.ckpt")
        ...
        while ((it < params.max_iter_m and converge > 1e-6) or it < min_iter):
            ...
            for i in xrange(len(users)):
                ...
                if n > 0:
                    ...
                    ep = self.U[i, :] - self.exp_z_u[i, :]
                    # likelihood += -0.5 * params.lambda_u * np.sum(self.U[i]*self.U[i])
                    likelihood += -0.5 * params.lambda_u * np.sum(ep*ep)
