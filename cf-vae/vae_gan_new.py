...
def build_model(d2d):
    ...
    print(z_mu_A.shape, z_mu_AB.shape)
    loss_kl_mu_A = tf.reduce_mean(tf.reduce_sum((z_mu_A-z_mu_BA)**2), axis=1)
    loss_kl_mu_B = tf.reduce_mean(tf.reduce_sum((z_mu_B-z_mu_AB)**2), axis=1)
