...
def build_model(d2d):
    ...
    loss_kl_mu_A = tf.reduce_mean(tf.reduce_sum(z_mu_A*tf.log(z_mu_A/z_mu_B), axis=1))
    loss_kl_mu_B = tf.reduce_mean(tf.reduce_sum(z_mu_B * tf.log(z_mu_B / z_mu_A), axis=1))
